...
function Register() {
  ...
  const register = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setLoading(true)
        const reg = await axios.post(check ? `${Config.api}/blogger/register` : `${Config.api}/user/register`, values)
        // const reg = await axios.post(`${Config.api}/user/register`, values)
        console.log(reg)
        if (reg.data.message === "blogger created" || reg.data.message === "user created") {
          setLoading(false)
          setDialog(true)
