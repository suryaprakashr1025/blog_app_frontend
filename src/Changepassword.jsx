...
function ChangePassword() {
  ...
  const changePassword = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setLoading(true)
        const change = await axios.put(check ? `${Config.api}/blogger/${changePassword.values.username}` : `${Config.api}/user/${changePassword.values.username}`, values)
  ...
  const navi = () => {
    nav ? navigate("/login") : navigate("/changepassword")
    setDialog(false)
  }
          ...
          <div className='row'>
            <div className='col-lg-12 mt-3 text-center'>
              <input class={`form-check-input ${dialog ? "form" : ""}`}
                ...
                value=""
                id="flexCheckDefault" />
              <span> <label> If you are Blogger?</label></span>
            </div>
